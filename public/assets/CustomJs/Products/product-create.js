...
$(document).ready(()=>{
  ...
  //Once we have the template of asingle we can add as many as we want
  divVersionTemplate = $('#divVersionTemplate');

  //getting the html to have a tempolate string
    ...
    $.ajax({
        ...
        error: (errorResponse)=>{
          ...
          for (const err in errorResponse.responseJSON.errors) {
            $.notify({
                title: err.toUpperCase()+' Error!',
                detail: errorResponse.responseJSON.errors[err]
              }, {
                style: 'detail'
              });
                // $.notify(`<h4>${err.toUpperCase()} Error!. </h4><br>${errorResponse.responseJSON.errors[err]}` , "error")
          }
      }
    })
